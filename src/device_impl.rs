    ...
    Delay: embedded_hal::delay::DelayNs,
{
    ...
    pub fn auto_stop(&mut self) -> Result<(), Error<E>> {
        self.cmd_and_read(&Command::AutoExit.as_be_bytes(), &mut [0u16; 0])?;
        Ok(())
    }
    ...
    pub fn heater(&mut self, heater_level: HeaterLevel) -> Result<(), Error<E>> {
        self.cmd_and_read(&Command::HeaterDisable.as_be_bytes(), &mut [0u16; 0])?;
        ...
        if let Some(setting) = heater_level.setting() {
            ...
            self.cmd_and_read(&Command::HeaterEnable.as_be_bytes(), &mut [0u16; 0])?;
        }
        Ok(())
    }
    ...
    pub fn read_status(&mut self, clear: bool) -> Result<StatusBits, Error<E>> {
        ...
        self.cmd_and_read(&Command::StatusRead.as_be_bytes(), &mut read_buf)?;
        if clear {
            self.cmd_and_read(&Command::StatusClear.as_be_bytes(), &mut [0u16; 0])?;
        }
    ...
    pub fn read_serial_number(&mut self) -> Result<SerialNumber, Error<E>> {
        ...
        let mut bytes= [0u8; 6];
        self.cmd_and_read(&Command::SerialID54.as_be_bytes(), &mut temp_u16)?;
        bytes[5] = (temp_u16[0] >> 8) as u8;
        bytes[4] = temp_u16[0] as u8;
        self.cmd_and_read(&Command::SerialID32.as_be_bytes(), &mut temp_u16)?;
        bytes[3] = (temp_u16[0] >> 8) as u8;
        bytes[2] = temp_u16[0] as u8;
        self.cmd_and_read(&Command::SerialID10.as_be_bytes(), &mut temp_u16)?;
    ...
    pub fn read_manufacturer_id(&mut self) -> Result<ManufacturerId, Error<E>> {
        let mut read_buf = [0u16; 1];
        self.cmd_and_read(&Command::ManufacturerID.as_be_bytes(), &mut read_buf)?;
        Ok(ManufacturerId::from(read_buf[0]))
    }

    /// software reset
    pub fn software_reset(&mut self) -> Result<(), Error<E>> {
        self.cmd_and_read(&Command::SoftReset.as_be_bytes(), &mut [0u16; 0])?;
        Ok(())
    }
    ...
    Delay: embedded_hal_async::delay::DelayNs,
{
    async fn cmd_and_read_async(&mut self, cmd_bytes: &[u8; 2], read_vals: &mut [u16]) -> Result<(), Error<E>> {
        ...
        if read_vals.is_empty() {
            ...
            if self.i2c.write_read(self.i2c_addr.as_u8(), cmd_bytes, read_buf_slice).await.is_err() {
                // TODO: consider a timeout and/or retry limit
                if self.i2c.read(self.i2c_addr.as_u8(), read_buf_slice).await.is_err() {
    ...
    pub async fn auto_stop_async(&mut self) -> Result<(), Error<E>> {
        self.cmd_and_read_async(&Command::AutoExit.as_be_bytes(), &mut [0u16; 0]).await?;
        Ok(())
    }
    ...
    pub async fn heater_async(&mut self, heater_level: HeaterLevel) -> Result<(), Error<E>> {
        self.cmd_and_read_async(&Command::HeaterDisable.as_be_bytes(), &mut [0u16; 0]).await?;
        ...
        if let Some(setting) = heater_level.setting() {
            ...
            self.cmd_and_read_async(&Command::HeaterEnable.as_be_bytes(), &mut [0u16; 0]).await?;
        }
        Ok(())
    }
    ...
    pub async fn read_status_async(&mut self, clear: bool) -> Result<StatusBits, Error<E>> {
        let mut read_buf = [0u16; 1];
        self.cmd_and_read_async(&Command::StatusRead.as_be_bytes(), &mut read_buf).await?;
        if clear {
            self.cmd_and_read_async(&Command::StatusClear.as_be_bytes(), &mut [0u16; 0]).await?;
        }
    ...
    pub async fn read_serial_number_async(&mut self) -> Result<SerialNumber, Error<E>> {
        let mut temp_u16 = [0u16; 1];
        let mut bytes= [0u8; 6];
        self.cmd_and_read_async(&Command::SerialID54.as_be_bytes(), &mut temp_u16).await?;
        bytes[5] = (temp_u16[0] >> 8) as u8;
        bytes[4] = temp_u16[0] as u8;
        self.cmd_and_read_async(&Command::SerialID32.as_be_bytes(), &mut temp_u16).await?;
        bytes[3] = (temp_u16[0] >> 8) as u8;
        bytes[2] = temp_u16[0] as u8;
        self.cmd_and_read_async(&Command::SerialID10.as_be_bytes(), &mut temp_u16).await?;
        bytes[1] = (temp_u16[0] >> 8) as u8;
        bytes[0] = temp_u16[0] as u8;
        Ok(SerialNumber(bytes))
    }
    ...
    pub async fn read_manufacturer_id_async(&mut self) -> Result<ManufacturerId, Error<E>> {
        let mut read_buf = [0u16; 1];
        self.cmd_and_read_async(&Command::ManufacturerID.as_be_bytes(), &mut read_buf).await?;
        Ok(ManufacturerId::from(read_buf[0]))
    }

    /// software reset
    pub async fn software_reset_async(&mut self) -> Result<(), Error<E>> {
        self.cmd_and_read_async(&Command::SoftReset.as_be_bytes(), &mut [0u16; 0]).await?;
        Ok(())
    }
